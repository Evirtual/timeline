...
export default function AgiWatchPage() {
  ...
  return (
    <div className="min-h-screen">
      {/* One column for the whole view — masthead, chart, gauge and cards on a
          single measure, centred. The race view centres its masthead on the
          same measure, so the title does not move when you switch. */}
          ...
          <div className="flex items-start justify-between gap-6">
            <div className="max-w-2xl">
              ...
              <p className="mt-3 text-balance leading-relaxed text-[var(--color-ink-muted)]">
                Seventy-six years of a field promising the same destination.
                This is not a release log — it carries only the moments that
                changed the answer to <em>are we getting there</em>: the
                capability thresholds, the setbacks that broke the last two
                schedules, and, increasingly, the claims that we have arrived.{" "}
                {data.milestones.length} entries in seventy-six years, so the
                ones that are here have to earn it.
              </p>
            </div>
      ...
      <footer className="mx-auto mt-10 max-w-[960px] border-t border-[var(--color-border)] px-4 py-8 md:px-8">
